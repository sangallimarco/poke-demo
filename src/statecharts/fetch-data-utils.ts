...
async function fetchAdditionalData<T>(id: number, endpoint: string, defaultValue: T): Promise<T> {
    try {
...
export async function fetchProcess(ctx: FetchContext): Promise<Partial<FetchContext>> {
    ...
    try {
        const resources = await fetch(`${ROOT_API}/pokemon?limit=${limit}&offset=${offset}`)
        ...
        const list: PokeData[] = resolvedPokemons.filter((pokemon) => !isNil(pokemon) && filterRegx.test(pokemon.name)) as PokeData[] // TS issue with filter operator
...
export async function fetchDetailsProcess(ctx: FetchContext): Promise<Partial<FetchContext>> {
