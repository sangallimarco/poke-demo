...
import { addItem, filterData, isInFavourites, mergeData, removeItem, reset, setFilter, setNextPage, setSelectedItem } from '../fetch-data-utils'
...
describe('helpers', () => {
  ...
  describe('setFilter()', () => {
    test('should set filter and reset offset', () => {
      const {filteredList = [], filter, offset} = setFilter(testContext, 'ampharos')
  ...
  describe('removeItem()', () => {
    ...
    const modifiedContext = {...testContext, favourites: [ampharosPokeData, jumpluffPokeData]}
  ...
  describe('isInFavourites()', () => {
    test('should return true if data is in favourites', () => {
      expect(isInFavourites( [ampharosPokeData, jumpluffPokeData], 189)).toBeTruthy()
      expect(isInFavourites( [ampharosPokeData, jumpluffPokeData], 2)).toBeFalsy()
    })
  })
})
